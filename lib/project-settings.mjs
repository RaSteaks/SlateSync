...
import { cleanTemplateName } from "../public/export-templates.js";
...
export function normalizeProjectSettings(value = {}, fallback = {}) {
  ...
  const exportTemplates = normalizeExportTemplates(merged.exportTemplates);
  const exportOptions = normalizeExportOptions(
    source.export ?? fallbackSource.export ?? DEFAULT_PROJECT_SETTINGS.export,
    source.export?.templateId || fallbackSource.export?.templateId ? base.export : DEFAULT_EXPORT_OPTIONS,
  );
  // A saved-template link is only meaningful while the referenced template
  // still exists and the config is not the implicit read-only built-in;
  // the effective export configuration itself is always preserved.
  if (exportOptions.savedTemplateId !== undefined && (
    typeof exportOptions.savedTemplateId !== "string"
    || !exportTemplates?.some((template) => template.id === exportOptions.savedTemplateId)
    || exportOptions.templateId === RESOLVE_TEMPLATE_ID
  )) {
    delete exportOptions.savedTemplateId;
  }
  const result = {
    ...merged,
    version: PROJECT_SETTINGS_VERSION,
  ...
  // Historical projects keep carrying no template library at all; an empty
  // list is only stored once the user actually creates or imports a template.
  if (exportTemplates) result.exportTemplates = exportTemplates;
  return result;
}
...
// Per-project template library. Corrupt entries drop individually instead of
// blocking the whole settings payload, and duplicate names keep only the
// first occurrence — the renderers block duplicate saves with a rename
// prompt before a payload ever reaches Main.
function normalizeExportTemplates(value) {
  ...
  for (const entry of value) {
    ...
    const id = typeof entry.id === "string" || typeof entry.id === "number"
      ? String(entry.id).trim().slice(0, 64)
      : "";
    const name = cleanTemplateName(entry.name);
    const templateId = entry.templateId === "custom" || entry.templateId === IMPORTED_TEMPLATE_ID
      ? entry.templateId
      : null;
    if (!id || seenIds.has(id) || !name || seenNames.has(name) || !templateId) continue;
    try {
      // Reuses the exact export-options pipeline so a template and the live
      // configuration never disagree about column or format normalization.
      const normalized = normalizeExportOptions(
        { templateId, columns: entry.columns, format: entry.format, filenameTemplate: entry.filenameTemplate },
        {},
      );
      seenIds.add(id);
      seenNames.add(name);
      ...
      // An imported template with invalid headers is dropped, not fatal.
    }
  }
  return templates;
}
...
/**
 * Task snapshots keep only the effective export configuration: the template
 * library stays project-owned, and the savedTemplateId link would dangle
 * once the library is stripped, so both are removed here.
 */
export function projectSettingsTaskSnapshot(value) {
  // Strip project-owned metadata before validation: stale libraries must not
  // affect restoration or validation of the task’s effective configuration.
  const { exportTemplates: _templateLibrary, ...snapshot } = isPlainRecord(value) ? value : {};
  // Preserve the absent-export default path for pre-export task snapshots.
  if (!isPlainRecord(snapshot.export)) return normalizeProjectSettings(snapshot);
  const { savedTemplateId: _templateLink, ...exportOptions } = snapshot.export;
  return normalizeProjectSettings({ ...snapshot, export: exportOptions });
}
...
function safeDelimiter(value, fallback = ",") {
  ...
  // Match the CSV encoder; migrate unsupported historical delimiters safely.
  const valid = (token) => typeof token === "string" && token.length === 1 && !/["\r\n\u0000]/.test(token);
  return valid(delimiter) ? delimiter : valid(fallback) ? fallback : ",";
}
