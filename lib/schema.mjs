...
export function normalizeSlateResult(value) {
  ...
  return {
    ...
    records: value.records.map((record, index) => ({
      ...
      videoCode: normalizeVideoCode(record.videoCode),
...
export function formatSlateResultFields(result, formats = {}) {
  ...
  return {
    ...
    records: result.records.map((record) => {
      ...
      // Keep older result shapes stable when optional identifier keys are not
      // present; normalize them only when the caller supplied the fields.
      if (Object.hasOwn(record, "cardNumber")) normalized.cardNumber = normalizeCardNumber(record.cardNumber);
      if (Object.hasOwn(record, "videoCode")) normalized.videoCode = normalizeVideoCode(record.videoCode);
      return normalized;
    }),
  };
}
...
// Canonicalize only unambiguous identifier shapes. Padding a numeric suffix
// makes model/OCR variants such as "A10" and "C 15" join the same material
// sequence, while malformed or ranged values remain untouched for review.
export function normalizeCardNumber(value) {
  const normalized = nullable(value)?.toUpperCase();
  if (!normalized) return null;
  const compact = normalized.replace(/[\s_-]+/g, "");
  const match = /^([A-Z])(\d{1,6})$/.exec(compact);
  if (!match) return normalized;
  const number = Number(match[2]);
  return Number.isSafeInteger(number)
    ? `${match[1]}${String(number).padStart(3, "0")}`
    : normalized;
}
...
export function normalizeVideoCode(value) {
  ...
  if (!match) return normalized;
  // Resolve material keys accept only C0XX. Reject numeric values such as
  // C115/C0115 instead of promoting them into a different valid-looking key.
  let digits = match[1];
