...
// (field-by-field mapping to Resolve Scene/Shot/Take/Comments), the core-audit
// variant, the system prompts that drive extraction, and the normalization
// applied to model output before it reaches the UI.
...
export function normalizeSlateResult(value, options = {}) {
  if (!value || typeof value !== "object" || !Array.isArray(value.records)) {
    throw new Error("模型返回的数据不包含 records 数组");
  }

  const normalizeFields = options.normalizeFields !== false;
  const fieldFormats = options.fieldFormats || { scene: "XXX", shot: "XX", take: "XX" };
  return {
    sheetTitle:
    ...
    records: value.records.map((record, index) => {
      const confidence = ["high", "medium", "low"].includes(record.confidence)
        ? record.confidence
        : "low";
      const normalizedField = (field) => normalizeFields
        ? normalizeRecognitionField(field, record[field], {
            fieldFormats,
            confidence,
          }).normalizedValue
        : rawNullable(record[field]);
      return {
        id: `record-${Date.now()}-${index}`,
        sourcePage: normalizeSourcePage(record.sourcePage),
        // Provider responses stay raw until ai-client has completed the page
        // merge, conflict review, sequence repair, and final target assignment.
        cardNumber: normalizedField("cardNumber"),
        videoCode: normalizedField("videoCode"),
        scene: normalizedField("scene"),
        shot: normalizedField("shot"),
        take: normalizedField("take"),
...
export function formatSlateResultFields(result, formats = {}) {
  ...
  // Preserve this public compatibility formatter's historical shape for
  // direct callers; ai-client runs the quality-producing shared gate before
  // entering here, so existing quality/review metadata is still retained.
  return normalizeRecognitionSheetFields(result, {
    fieldFormats: {
      scene: formats.scene || "XXX",
      shot: formats.shot || "XX",
      take: formats.take || "XX",
    },
    createQuality: false,
  });
}
...
function rawNullable(value) {
  if (value == null) return null;
  const raw = String(value);
  return raw.trim() ? raw : null;
}

// These exports remain for older callers. The shared helper makes the decision;
// the small fallback preserves the historical return shape for malformed
// values that callers used to display directly rather than persist as quality.
export function normalizeCardNumber(value) {
  const result = normalizeRecognitionField("cardNumber", value);
  if (isCanonicalRecognitionValue("cardNumber", result.normalizedValue) &&
    !result.warnings.some((item) => ["ambiguous-numeric-token", "invalid-numeric-token", "out-of-range"].includes(item.code))) {
    return result.normalizedValue;
  }
  return nullable(value)?.toUpperCase() || null;
}

export function normalizeVideoCode(value) {
  const result = normalizeRecognitionField("videoCode", value);
  if (isCanonicalRecognitionValue("videoCode", result.normalizedValue) &&
    !result.warnings.some((item) => ["ambiguous-numeric-token", "invalid-numeric-token", "out-of-range"].includes(item.code))) {
    return result.normalizedValue;
  }

  // Keep the old null-for-invalid-number adapter contract for direct imports;
  // final records use normalizeRecognitionField and therefore retain raw input.
  const normalized = nullable(value)?.toUpperCase();
  if (!normalized) return null;
  const compact = normalized.replace(/\s+/g, "");
  const match = /^(?:C)?(\d+)$/.exec(compact);
  if (!match) return normalized;
  let digits = match[1];
  while (digits.length > 3 && digits.startsWith("0")) digits = digits.slice(1);
  if (digits.length > 3) return null;
  digits = digits.padStart(3, "0");
  return digits.startsWith("0") ? `C${digits}` : null;
}
