    ...
    records: {
      ...
      items: {
        ...
        properties: {
          ...
          scene: {
            ...nullableString,
            description: "场记单“场次”列，将写入 Resolve Scene。只返回场次中的数字并补足三位，例如“37A”返回“037”，“1”返回“001”；不要返回字母、“场、场次、第”等文字、空格或标点。如果场次是复合格式（如“16/72A”“54/55A”），只返回斜杠后最后一个数字部分：例如“16/72A”返回“072”，“54/55A”返回“055”。",
          },
          shot: {
            ...nullableString,
...
- 场记单条次状态符号 → Resolve Comments（☑/√/✓ 写入“_OK”，三角形/△ 写入“_KP”，X/× 或未标记时清空；CSV 中绝不写入“过”“保”“废条”等字样）
...
8. 场次 scene 只保留数字并固定为三位：例如“37A”输出“037”，“第1场”输出“001”。如果场次是复合格式（如“16/72A”“54/55A”），只返回斜杠后最后一个数字部分并补足三位：例如“16/72A”返回“072”，“54/55A”返回“055”；不要返回斜杠前的数字。镜 shot 和次 take 只保留数字并固定为两位：例如镜 2 输出“02”，镜 18 输出“18”，次 9 输出“09”。两位镜号 10–19 的左侧“1”可能贴近竖线或写得很细，必须完整保留：若上一镜为 17、下一组手写为 18，绝不能只读成 08。输出前按同一场次复查：如果连续素材的 shot 跟着每行“次”一起递增，而 take 却反复为 "01"，说明把“次”误读成了“镜”；如果同一镜组中只有一行 shot 与前后行不同而 take 连续递增，说明该行镜号读错；必须返回表格重新按列边界和合并镜号纠正。
...
4. scene 只保留数字并补足三位；复合场次取斜杠后最后一段。shot 和 take 只保留数字并补足两位，尤其不能漏掉 10–19 的十位。
