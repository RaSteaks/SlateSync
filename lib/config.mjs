...
import { DEFAULT_SCENARIO_MATCHING } from "./scenario/profile.mjs";
...
export const DEFAULT_WORKFLOW_CONFIG = Object.freeze({
  ...
  scenario: Object.freeze({
    matching: Object.freeze({ ...DEFAULT_SCENARIO_MATCHING }),
  }),
...
export function resolveProvider(providerId, env = process.env) {
  ...
  const { transport, jsonMode: chatJsonMode } = normalizeLegacyCompatibleOptions(
    env[provider.transportEnv],
    env[provider.jsonModeEnv],
  );

  return {
    ...provider,
    transport,
    // Responses has always used the strict schema contract; preserve that
    // legacy default while Chat Completions keeps its json_object fallback.
    chatJsonMode,
  };
}

/**
 * Normalize the legacy OpenAI-compatible environment contract before it is
 * copied into a v2 provider record. Responses historically required the
 * strict schema format even when the old setting said json_object.
 */
export function normalizeLegacyCompatibleOptions(transportValue, jsonModeValue) {
  const transport = normalizeCompatibleTransport(transportValue);
  const requestedJsonMode = normalizeCompatibleJsonMode(jsonModeValue, transport);
  return {
    transport,
    jsonMode: transport === "responses" && requestedJsonMode === "json_object"
      ? "json_schema"
      : requestedJsonMode,
  };
}
...
export function resolveModel(providerId, requestedId, env = process.env) {
  ...
  // A discovered compatible custom descriptor is tied to one API model. Do
  // not let a stale registration override a newer global model setting.
  const compatibleModelId = providerId === "openai-compatible"
    ? cleanEnv(env.OPENAI_COMPATIBLE_MODEL)
    : "";
  if (
    discovered &&
    (providerId !== "openai-compatible" || requestedId !== CUSTOM_MODEL_ID || discovered.apiId === compatibleModelId)
  ) return discovered;
...
export function normalizeWorkflowConfig(value = {}) {
  ...
  const matching = normalizeScenarioMatching(value.scenario?.matching);
...
export function publicConfig(
  ...
  const paddleStatus = paddleOcrPublicConfig(env, {
    autoEnable: ocrAutoEnable,
  });
  const visionStatus = visionOcrPublicConfig(env, {
    autoEnable: ocrAutoEnable,
  });
  const selection = resolveOcrSelection(env, {
    autoEnable: ocrAutoEnable,
    paddle: paddleStatus,
    vision: visionStatus,
  });
  const { pythonPath: _privatePythonPath, ...ocr } = paddleStatus;
  const { binaryPath: _privateBinaryPath, ...vision } = visionStatus;
  return {
    ...
    // Derive public rating provenance before stripping raw price fields; the
    // renderer needs dates/source but never receives token prices themselves.
    models: [...MODELS, compatibleModel(env)].map(withRatingMetadata).map(withoutPricing),
    ocr,
    ocrEngines: [ocr, vision],
    ocrSelection: {
      id: selection.id,
      label: selection.id ? selection.engine.label : "未启用本地 OCR",
      mode: selection.mode,
      reason: selection.reason,
      available: Boolean(selection.id && selection.engine.available),
      enabled: Boolean(selection.id && selection.engine.enabled),
      required: Boolean(selection.id && selection.engine.required),
    },
    upload: {
      acceptedTypes: [
        "image/jpeg",
...
function normalizeCompatibleJsonMode(value, transport = "chat-completions") {
  const normalized = String(value || "").trim().toLowerCase();
  if (["json_schema", "json_object", "prompt"].includes(normalized)) {
    return normalized;
  }
  return transport === "responses" ? "json_schema" : "json_object";
}
...
function normalizeScenarioMatching(value = {}) {
  const threshold = Number(
    value.threshold ?? DEFAULT_SCENARIO_MATCHING.threshold,
  );
  const ambiguityMargin = Number(
    value.ambiguityMargin ?? DEFAULT_SCENARIO_MATCHING.ambiguityMargin,
  );
  if (!Number.isFinite(threshold) || threshold < 0.5 || threshold > 1) {
    throw new Error("scenario.matching.threshold 必须是 0.5–1 之间的数字");
  }
  if (
    ...
    throw new Error(
      "scenario.matching.ambiguityMargin 必须是 0–0.5 之间的数字",
    );
  }
  return { threshold, ambiguityMargin };
}
...
function withoutPricing(model) {
  ...
  // Price data stays Main-only; the renderer receives only rating provenance
  // and dates through the metadata fields added below.
  delete publicModel.priceUpdatedAt;
  return publicModel;
}

function withRatingMetadata(model) {
  const datedCatalogPrice = model.priceUpdatedAt && model.pricePerMillion
    ? model.pricePerMillion.openai || model.pricePerMillion.openrouter || Object.values(model.pricePerMillion)[0]
    : null;
  const valueScore = datedCatalogPrice
    ? model.valueScore ?? calculateValueScore(
      model.qualityScore,
      datedCatalogPrice.input,
      datedCatalogPrice.output,
    )
    : null;
  return {
    ...model,
    qualityLabel: qualityLabel(model.qualityScore),
    qualitySource: model.qualityScore != null ? "SlateSync 维护的模型族参考评级" : null,
    qualityUpdatedAt: model.qualityScore != null ? (model.priceUpdatedAt || PRICE_CATALOG_DATE) : null,
    valueScore,
    valueLabel: valueLabel(valueScore),
    valueSource: valueScore != null ? (model.priceUpdatedAt ? "内置价格目录" : null) : null,
    valueUpdatedAt: valueScore != null ? (model.priceUpdatedAt || null) : null,
  };
}

const PRICE_CATALOG_DATE = "2026-08-02";
