...
// Shared adapters preserve Swift encryption for databases and JSON snapshots.
import { openCompatibleDatabase as Database } from "./encrypted-database.mjs";
...
} from "./project-library-transfer.mjs";

export const LIBRARY_FORMAT_VERSION = 1;
export const PROJECT_FORMAT_VERSION = 1;
export const LIBRARY_EXTENSION = ".slatesync-library";
export const PROJECT_PACKAGE_EXTENSION = ".slatesync-project";
export const PROJECT_PACKAGE_MANIFEST = "slatesync-project.json";
export const DEFAULT_LIBRARY_NAME = "Local SlateSync Library";
export const DEFAULT_LIBRARY_FOLDER = DEFAULT_LIBRARY_NAME;
export const LEGACY_DEFAULT_LIBRARY_FOLDER =
  `${DEFAULT_LIBRARY_NAME}.slatesync-library`;
...
export function defaultLibraryPath(applicationSupportPath) {
  // The built-in local Library is an application-data directory, not an
  // exported package, so its display name does not need a portable suffix.
  return join(applicationSupportPath, DEFAULT_LIBRARY_FOLDER);
}

export async function migrateDefaultLibraryPath(
  applicationSupportPath,
  legacyPaths = [],
  { preserveLegacyOnConflict = false } = {},
) {
  const preferred = defaultLibraryPath(applicationSupportPath);
  const preferredExists = await exists(preferred);
  if (preferredExists && !preserveLegacyOnConflict) return preferred;

  // Rename only known default locations. User-selected portable Libraries keep
  // their .slatesync-library name and continue to follow the import contract.
  for (const legacyPath of legacyPaths) {
    if (!legacyPath || !(await exists(legacyPath))) continue;
    // An explicitly configured legacy path remains authoritative when both
    // folders exist; silently choosing the short path can surface other data.
    if (preferredExists) return legacyPath;
    try {
      await rename(legacyPath, preferred);
      return preferred;
    } catch {
      // A read-only Application Support directory must not make the user's
      // existing Library unavailable merely because its name cannot migrate.
      return legacyPath;
    }
  }
  return preferred;
...
export function createProjectLibrary(libraryRoot, options = {}) {
  let rootPath = resolve(String(libraryRoot || ""));
  let projectsPath = join(rootPath, "Projects");
  const openedDatabase = openSlateDatabase(rootPath, { kind: "library" });
  const { db } = openedDatabase;
  let { dbPath } = openedDatabase;
  ...
  const store = {
    ...
    async renameLibrary(nextName) {
      await ready;
      const name = validateLibraryName(nextName);
      if (name === manifest.name) return store.getLibraryInfo();
      const oldPath = rootPath;
      const parentDir = dirname(oldPath);
      const newFolder = basename(oldPath).endsWith(LIBRARY_EXTENSION)
        ? `${name}${LIBRARY_EXTENSION}`
        : name;
      const newPath = join(parentDir, newFolder);
      if (await exists(newPath)) {
        throw new Error("该名称的项目库目录已存在，请选择其他名称");
      }
      const nextManifest = { ...manifest, name };
      // Move the directory before committing metadata. If the metadata write
      // fails, restore the old path so a reported failure leaves no half-rename.
      await renameDirectory(oldPath, newPath);
      try {
        await writeJsonAtomic(join(newPath, "library.json"), nextManifest);
      } catch (error) {
        try {
          await renameDirectory(newPath, oldPath);
        } catch (rollbackError) {
          const combined = new Error(`项目库改名失败，且无法恢复原目录：${rollbackError?.message || rollbackError}`);
      ...
      // The SQLite connection keeps the same inode across a POSIX directory
      // rename, so the open handles stay valid while we point the store at the
      // new root for all subsequent project-path resolution.
      rootPath = newPath;
      projectsPath = join(newPath, "Projects");
      dbPath = join(newPath, basename(dbPath));
      // Encrypted connections reload by path rather than retaining a file inode.
      db.relocatePath?.(dbPath);
      store.libraryRoot = rootPath;
      store.projectsPath = projectsPath;
      store.dbPath = dbPath;
      return store.getLibraryInfo();
    },
    ...
    async listProjects({ includeArchived = false } = {}) {
      ...
      `).all();
      // Keep only one temporary decrypted project database alive at a time.
      const summaries = [];
      for (const row of rows) summaries.push(await projectSummary(row));
      return summaries;
    },

    async getProject(id, { allowArchived = true } = {}) {
      return projectSummary(await getProjectRow(id, { allowArchived }), {
        includeSettings: true,
      });
    },

    // 项目运行时的两个廉价入口：索引行解析（不碰 project.sqlite）与
    // 可注入共享连接的完整摘要，配合每项目单一 SQLite 句柄使用。
    getProjectRow,
    summarizeProjectRow: projectSummary,
    ...
    async updateProject(id, patch = {}) {
      ...
      const settings = patch.settings === undefined
        ? current.settings
        // Renderer updates may still be v1-shaped; current settings provide
        // the compatibility fallback for export options and future branches.
        : validateProjectSettings(patch.settings, current.settings);
      const now = new Date().toISOString();
      const projectDb = openSlateDatabase(projectDir, {
        kind: "project",
        filename: SQLITE_FILENAMES.project,
    ...
    // 项目导出读取索引中的归档/活动状态，再由传输层生成独立目录包。
    async exportProjectPackage(id, targetPath) {
      await ready;
      const current = await store.getProject(id);
      await exportProjectPackage(current.directoryPath, targetPath, {
        id: current.id,
        name: current.name,
    ...
    // 导入先完成包校验和新目录落盘，再插入索引，避免出现悬空项目记录。
    async importProjectPackage(packagePath) {
      await ready;
      const packageInfo = await validateProjectPackage(packagePath);
      const projectId = await nextImportedProjectId(packageInfo.project.id);
      const projectDir = projectDirectory(projectId);
      await importProjectPackageData(packageInfo.path, projectDir, {
        projectId,
        libraryId: manifest.id,
        // Reuse the validation result above; the transfer layer still validates
        // independently when called directly by another integration.
        packageInfo,
      });
      try {
        db.prepare(`
        ...
        `).run(
          projectId,
          relative(rootPath, projectDir),
          packageInfo.project.name,
          packageInfo.project.description,
          packageInfo.project.archivedAt,
          packageInfo.project.createdAt,
          packageInfo.project.updatedAt,
        );
      } catch (error) {
        // 项目目录先落盘再写索引；索引失败时清理未登记副本，删除受锁时交给启动重试。
        await cleanupFailedImportedProject(projectDir);
        throw error;
      }
      ...
      try {
        ...
        if (await exists(legacyDbPath)) {
          ...
          const transaction = target.db.transaction(() => {
            ...
            // 批量导入绕过 task-store 的增量维护：删除默认值键强制下次读取
            // 重新扫描，避免旧键遮蔽迁移进来的更新任务。
            if (counts.tasks > 0) invalidateRecognitionDefaults(target.db);
          });
          transaction();
        }
      } finally {
  ...
  // 仅查 library 索引行并做归档/路径校验，绝不打开 project.sqlite。
  // 项目运行时用它做每请求的廉价刷新（归档、删除、改名即时生效），
  // 返回值在原始行上补充 directoryPath 供运行时打开共享连接。
  async function getProjectRow(id, { allowArchived = false } = {}) {
    await ready;
    const row = findProjectRow(id);
    if (!row) throw missingProject();
    if (!allowArchived && row.archived_at) throw archivedProject();
    return { ...row, directoryPath: checkedProjectDirectory(row) };
  }

  async function projectSummary(row, { db = null, includeSettings = false } = {}) {
    const projectDir = row.directoryPath || checkedProjectDirectory(row);
    let taskCount = 0;
    let latestTaskAt = null;
    let settings = normalizeProjectSettings(options.defaultSettings);
    let lastRecognitionDefaults = null;
    // 注入共享句柄（运行时路径）直接复用连接；否则仅在 project.sqlite 已
    // 存在时临时开库，list-projects 等轻量路径保持不变。
    if (db) {
      ({ taskCount, latestTaskAt, settings, lastRecognitionDefaults } =
        collectTaskSummary(db, includeSettings));
    } else if (await exists(join(projectDir, SQLITE_FILENAMES.project))) {
      const projectDb = openSlateDatabase(projectDir, {
        kind: "project",
        filename: SQLITE_FILENAMES.project,
      });
      try {
        ({ taskCount, latestTaskAt, settings, lastRecognitionDefaults } =
          collectTaskSummary(projectDb.db, includeSettings));
      } finally {
        closeSlateDatabase(projectDb.db);
      }
    }
  ...
  // project.sqlite 内的廉价统计：COUNT/MAX + （按需）settings 与识别默认值。
  // 调用方保证传入已打开的 project 连接；运行时路径每个项目只执行一次。
  function collectTaskSummary(projectDb, includeSettings) {
    const summary = projectDb.prepare(
      // Separate aggregates let SQLite count the small index and seek its last
      // timestamp; combining COUNT/MAX forces a per-row aggregate scan.
      `SELECT (SELECT COUNT(*) FROM tasks) AS count,
              (SELECT MAX(updated_at) FROM tasks) AS latest`,
    ).get();
    const collected = {
      taskCount: Number(summary?.count) || 0,
      latestTaskAt: summary?.latest || null,
      settings: normalizeProjectSettings(options.defaultSettings),
      lastRecognitionDefaults: null,
    };
    if (includeSettings) {
      collected.settings = readProjectSettings(projectDb, options.defaultSettings);
      collected.lastRecognitionDefaults = readLastRecognitionDefaults(projectDb);
    }
    return collected;
  }
  ...
  async function nextImportedProjectId(sourceId) {
    let candidate = idForProject(`${sourceId}:${randomUUID()}`);
    while (findProjectRow(candidate) || await exists(projectDirectory(candidate))) {
      candidate = idForProject(`${sourceId}:${randomUUID()}`);
    }
    return candidate;
  }

  async function cleanupFailedImportedProject(projectDir) {
    try {
      await rm(projectDir, { recursive: true, force: true });
    } catch {
      // 删除失败也不能留下可寻址项目；启动时会清理这个 tombstone 目录。
      try {
        await rename(projectDir, `${projectDir}.deleting-${randomUUID()}`);
      } catch {
        // 若连改名也失败，只能保留原目录等待外部文件锁释放。
      }
    }
  }
...
function readProjectSettings(db, fallback) {
  ...
  try {
    ...
    // A future settings version must be surfaced instead of being silently
    // replaced with defaults, while malformed legacy JSON remains recoverable.
    if (error?.code === "UNSUPPORTED_PROJECT_SETTINGS_VERSION") throw error;
    return normalizeProjectSettings({}, fallback);
  }
}

// readLastRecognitionDefaults 已迁移到 lib/recognition-defaults.mjs：
// 键值 O(1) 命中 + 回退扫描，由 task-store 在写事务内增量维护。
...
function validateLibraryName(value) {
  const name = String(value || "").trim();
  if (!name) throw new Error("项目库名称不能为空");
  if (name.length > 80) throw new Error("项目库名称不能超过 80 个字符");
  // The name doubles as a folder name on disk, so path separators and the
  // reserved current/parent entries are rejected up front.
  if (/[/\\:<>"|?*\u0000-\u001f]/.test(name)) {
    throw new Error("项目库名称不能包含 / \\ : 等特殊字符");
  }
  if (name === "." || name === "..") throw new Error("项目库名称无效");
  return name;
}
