...
export function createTaskStore(baseDir, options = {}) {
  ...
  const ready = (async () => {
    ...
    const imported = await migrateJsonDirectory({
      ...
      // Direct inserts bypass saveTask. Invalidate in the import transaction,
      // so neither interruptions nor empty migrations leave stale defaults or
      // cause an unnecessary full encrypted database rewrite.
      onImported: () => invalidateRecognitionDefaults(db),
  ...
  const store = {
    ...
    async listTasks() {
      ...
      for (const row of rows) {
        ...
        try {
          // Healthy summaries must never materialize image/CSV payloads in JS.
          // Fetch the full task only when this particular summary needs repair.
          readPayload ||= db.prepare("SELECT data_json FROM tasks WHERE id = ?");
          const payload = readPayload.get(row.id);
          if (!payload) continue;
          const data = JSON.parse(payload.data_json);
