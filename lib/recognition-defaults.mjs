...
// Cache a completed scan with no eligible task too. Draft-only projects must
// not parse every image/CSV payload on each subsequent project read.
const EMPTY_DEFAULTS = Object.freeze({ empty: true });

// 公开摘要形状：与旧扫描实现返回值逐字段一致，测试与 IPC 契约都依赖它。
function publicDefaults(stored) {
  if (!stored || stored.empty === true) return null;
  return {
...
function readStoredDefaults(db) {
  ...
  try {
    ...
    if (value?.empty === true) return EMPTY_DEFAULTS;
...
export function readLastRecognitionDefaults(db) {
  ...
  if (!stored) {
    ...
    // The sentinel survives reopen; successful saves replace it in the same
    // transaction as the task, while bulk imports explicitly invalidate it.
    writeStoredDefaults(db, stored || EMPTY_DEFAULTS);
  }
...
export function recordRecognitionDefaults(db, task, rowid) {
  ...
  const wins = !stored || stored.empty === true
