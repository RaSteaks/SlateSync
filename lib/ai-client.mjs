...
// multi-stage "high accuracy" audit, PDF paging, progress events, and cost
// tracking.
...
export async function recognizeSlate(
  ...
  if (hasPdf && provider.supportsDirectPdf === false) {
    throw clientError(
      `${provider.label} 需要先把 PDF 转为页面图片；请使用网页上传，或通过 imageDataGroups 提交逐页图像`,
      400,
    );
  }
  if (!hasPdf && (
  ...
  const formatNote = fieldFormatInstruction(fieldFormats) + commentsInstruction(comments);
  const customContext = withCustomPrompt(SYSTEM_PROMPT, customPrompt, slateCsvContext) + formatNote;
  const auditContext = withCustomPrompt(CORE_AUDIT_SYSTEM_PROMPT, customPrompt, slateCsvContext) + formatNote;
  const reviewContext = withCustomPrompt(CORE_REVIEW_SYSTEM_PROMPT, customPrompt, slateCsvContext) + formatNote;
  const pdfContext = withCustomPrompt(PDF_SYSTEM_PROMPT, customPrompt, slateCsvContext) + formatNote;
  ...
  reportProgress({
    ...
    message: ocrSummary.used
      ...
      : ocrSummary.warning
        ? `${engineName} 已降级，继续使用多模态模型`
        : `未启用 ${engineName}，继续使用多模态模型`,
  });
  if (hasPdf) {
    let response;
    let normalized;
    ...
    const result = formatSlateResultFields(
      mergePdfResult(normalized, normalizedPageCount, accuracyMode),
      fieldFormats,
    );
      ...
      try {
        ...
        if (!highAccuracy) {
          ...
          if (capture) {
            capture.addPageResult(pageNumber, "primary", {
              request: { systemPrompt: customContext, ocrEvidence: fullOcrEvidence, filename: pageFilename },
        ...
        if (capture) {
          ...
          capture.addPageResult(pageNumber, "audit", {
            request: { systemPrompt: auditContext, ocrEvidence: coreOcrEvidence, filename: `${pageFilename} · 核心字段查漏`, viewCount: corePageImages.length },
        ...
        if (combined.conflicts.length || combined.auditOnly.length) {
          ...
          if (capture) {
            capture.addPageResult(pageNumber, "review", {
              request: { systemPrompt: reviewContext, userInstruction: `只复核以下 ${reviewTargets.length} 个素材键`, ocrEvidence: coreOcrEvidence, filename: `${pageFilename} · 冲突复核`, viewCount: corePageImages.length },
  ...
  if (ocrSummary.warning) {
    ...
    result.warnings.unshift(
      "本地 OCR 已运行但未检测到文字，当前结果主要依赖多模态图像识别。",
    );
  }
...
function mergePdfResult(result, pageCount, accuracyMode) {
...
async function postChatCompletions(provider, payload, headers, fetchImpl, timeoutMs) {
  const data = await postJson(
...
async function postJson(url, payload, headers, fetchImpl, timeoutMs) {
  ...
  try {
    ...
    if (error.name === "TimeoutError") {
      ...
      throw providerError(`模型请求超时（已等待 ${waited} 秒），请稍后重试`, 504);
    }
...
function skippedDirectPdfOcr(env) {
  const engine = resolveOcrEngine(env, {});
  const status = paddleOcrPublicConfig(env, { autoEnable: env === process.env });
  if (engine.meta.enabled && engine.meta.required) {
    const requiredEnv = engine.meta.id === "vision"
      ? "VISIONOCR_REQUIRED"
      : "PADDLEOCR_REQUIRED";
    throw clientError(
      `当前接口直接提交的 Base64 PDF 无法运行必需的 ${OCR_ENGINE_NAMES[engine.meta.id] || "本地 OCR"}；请改为逐页图片，或关闭 ${requiredEnv}。`,
      400,
    );
  }
  return {
    ...
    warning: status.enabled
      ? "直接提交 Base64 PDF 的兼容路径未运行本地 OCR；网页上传会先转成页面图片并启用 OCR。"
      : null,
  };
}
...
export function resolveOcrEngine(env, options = {}) {
  ...
  const vision = visionOcrPublicConfig(env, { autoEnable });
  const paddle = paddleOcrPublicConfig(env, { autoEnable });
  const visionMode = explicitOcrMode(env.VISIONOCR_ENABLED);
  const paddleMode = explicitOcrMode(env.PADDLEOCR_ENABLED);
  if (visionMode.enabled) return { impl: runVisionOcrForPages, meta: vision };
  if (paddleMode.enabled) return { impl: runPaddleOcrForPages, meta: paddle };
  if (visionMode.disabled) return { impl: runPaddleOcrForPages, meta: paddle };
  if (vision.enabled) return { impl: runVisionOcrForPages, meta: vision };
  return { impl: runPaddleOcrForPages, meta: paddle };
}

function explicitOcrMode(value) {
