...
export function resolveOcrEngine(env, options = {}) {
  ...
  // A required engine must remain the selected engine even when it is not
  // installed; otherwise auto-selection could silently bypass strict mode by
  // switching to the other local OCR implementation.
  if (vision.required) return { impl: runVisionOcrForPages, meta: vision };
  if (paddle.required) return { impl: runPaddleOcrForPages, meta: paddle };
