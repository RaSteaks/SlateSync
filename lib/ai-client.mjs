...
import { recognitionTargetId } from "../public/recognition-target.js";
import {
...
export async function recognizeSlate(
  ...
  const resolvedProvider = registry?.resolveProvider(providerId) || resolveProvider(providerId, env);
  ...
  const provider = {
    ...
    baseUrl: normalizeBaseUrl(
      resolvedProvider.baseUrl || env[resolvedProvider.baseUrlEnv] || resolvedProvider.defaultBaseUrl,
      resolvedProvider.baseUrlEnv || "Base URL",
    ),
    timeoutMs: modelRequestTimeoutMs(env),
    maxRetries: modelRequestMaxRetries(env),
  };
  // When Main supplies a registry, a null result is authoritative: falling
  // back to the legacy resolver here could resurrect a model whose custom
  // probe failed or whose connection revision was invalidated.
  const model = registry
    ? registry.resolveModel(providerId, modelId)
    : resolveModel(providerId, modelId, env);
  if (!model) {
  ...
  // Crop evidence is an opt-in output contract. Keeping it out of the
  // default OCR payload preserves the old block shape and avoids image
  // encoding/caching work when Phase 06 is disabled.
  const cropConfig = normalizeCropRecheckConfig({
    enabled: env.SLATESYNC_CROP_RECHECK,
    maxTargets: env.SLATESYNC_CROP_RECHECK_MAX_TARGETS,
  });
  const includeCropEvidence = highAccuracy && cropConfig.enabled && cropConfig.maxTargets > 0;
  if (
  ...
  try {
    ...
    ocrResult = await ocrImpl(imageGroups, {
      ...
      includeCrops: includeCropEvidence,
    });
  } catch (error) {
  ...
  let ocrSummary = {
    ...
    preprocessVersion: preprocessMetadata?.version || summarizedOcr.preprocessVersion || null,
    preprocessApplied: Boolean(preprocessMetadata?.applied || summarizedOcr.preprocessApplied),
    preprocessFallbackCount: Number(preprocessMetadata?.fallbackCount || summarizedOcr.preprocessFallbackCount) || 0,
    deskewApplied: Boolean(preprocessMetadata?.deskewApplied || summarizedOcr.deskewApplied),
    preprocessDurationMs: Number(preprocessMetadata?.durationMs || summarizedOcr.preprocessDurationMs) || 0,
  };
  ...
  // Page responses remain raw through merge/conflict/sequence repair. The
  // shared final gate creates field quality exactly once after targetId exists;
  // the historical formatter is then retained as a no-op compatibility
  // adapter for callers that still import its old name.
  const mergedResult = normalizeRecognitionSheetFields(
    mergePageResults(pages, accuracyMode),
    { fieldFormats: effectiveFieldFormats },
  );
  let result = formatSlateResultFields(mergedResult, effectiveFieldFormats);
  if (ocrDegradedWarning) result.warnings.unshift(ocrDegradedWarning);

  // Crop recheck is an explicit high-accuracy opt-in. It only consumes crops
  // emitted by the local OCR engine, so a missing bbox/crop is a safe skip and
  // never becomes a guessed field value.
  let cropRecheck = emptyCropRecheckSummary(cropConfig.enabled && highAccuracy);
  if (cropConfig.enabled && highAccuracy) {
    try {
      const rechecked = await runCropRecheck({
        records: result.records,
        ocrResult,
        imageDataGroups: imageGroups,
        config: cropConfig,
        provider,
        model,
        apiKey,
      ...
      result = { ...result, records: rechecked.records };
      cropRecheck = rechecked.summary;
      if (cropRecheck.warning) result.warnings.push(cropRecheck.warning);
    } catch (error) {
      if (signal?.aborted) throw recognitionCanceledError();
      cropRecheck = { ...cropRecheck, warning: `裁剪复核未完成，已保留原识别结果：${error?.message || "未知错误"}` };
      result.warnings.push(cropRecheck.warning);
    }
  }
  ocrSummary = { ...ocrSummary, cropRecheck };
  // The first summary is useful for early diagnostic progress; publish the
  // final summary again after the optional recheck so persisted sessions carry
  // the actual attempted/confirmed counts.
  if (capture) capture.setOcrResult(ocrSummary, ocrResult);
...
async function runCropRecheck({
  ...
  const selection = selectCropRecheckTargets({ records, ocrResult, imageDataGroups, accuracyMode: "high" }, config);
  const batches = batchCropRecheckTargets(selection.targets, config.batchSize);
  onProgress?.({
    phase: "crop-select",
    percent: 97,
    message: `已选择 ${selection.targets.length} 个裁剪复核目标，跳过 ${selection.skipped} 个`,
    completed: 0,
    total: selection.targets.length,
  });
  const summary = {
    ...emptyCropRecheckSummary(true),
    skipped: selection.skipped,
    deduplicatedCount: selection.deduplicated || 0,
  ...
  for (const batch of batches) {
    ...
    const batchResults = await Promise.all(batch.map(async (target) => {
      ...
      try {
        const response = await withCropRecheckTimeout(callImageRecognition({
          provider,
          model,
          apiKey,
          imageDataUrls: [target.cropImage, target.coreImage],
          imageViewMode: "core",
          filename: `${filename} · ${target.field} 裁剪复核`,
          schema: CORE_SLATE_SCHEMA,
          systemPrompt,
          userInstruction: `只复核 targetId=${target.targetId} 的字段 ${target.field}。原识别值为 ${target.currentValue || "空"}，OCR 坐标为 [${target.bboxNormalized.join(", ")} ]。确认图片中的字段后返回该字段；无法确认时不要猜测。`,
          env,
          fetchImpl,
          signal: requestController.signal,
        }), config.timeoutMs, signal, requestController);
        const candidate = response?.result?.records?.find((item) => item?.[target.field] != null) || response?.result?.records?.[0];
        const value = candidate?.[target.field] == null ? "" : String(candidate[target.field]).trim();
        const confidence = ["high", "medium", "low"].includes(candidate?.confidence) ? candidate.confidence : null;
        return {
          targetId: target.targetId,
          sourcePage: target.sourcePage,
          field: target.field,
          value,
          status: value ? (confidence === "low" ? "uncertain" : "confirmed") : "not-found",
          confidence,
          warningCode: null,
          durationMs: Date.now() - startedAt,
        };
        ...
        if (signal?.aborted) throw recognitionCanceledError();
        if (error?.code === "CROP_RECHECK_TIMEOUT") summary.timedOut = true;
        return { targetId: target.targetId, sourcePage: target.sourcePage, field: target.field, status: "failed", value: null, confidence: null, warningCode: error?.code || "provider-error", durationMs: Date.now() - startedAt };
      } finally {
    ...
    onProgress?.({
      phase: "crop-recheck",
      percent: 97 + Math.round((results.length / Math.max(1, selection.targets.length)) * 2),
      message: `裁剪复核 ${results.length}/${selection.targets.length}`,
      completed: results.length,
      total: selection.targets.length,
    });
  }
  const applied = applyCropRecheckResults(records, results, selection.targets, { fieldFormats });
  const rejected = Math.max(results.filter((item) => item.status !== "confirmed").length, applied.rejected);
  const output = {
    ...
    summary: {
      ...
      warning: rejected ? `有 ${rejected} 个裁剪复核候选未被确认，已保留原值。` : null,
    },
  };
  onProgress?.({
    phase: "crop-complete",
    percent: 99,
    message: `裁剪复核完成：确认 ${output.summary.confirmed}，跳过/拒绝 ${output.summary.rejected + output.summary.skipped}`,
    completed: output.summary.confirmed,
    total: selection.targets.length,
  });
  return output;
}

function withCropRecheckTimeout(promise, timeoutMs, signal, requestController = null) {
  const timeout = Math.max(5_000, Number(timeoutMs) || 120_000);
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      const error = new Error("裁剪复核请求超时");
      error.code = "CROP_RECHECK_TIMEOUT";
      reject(error);
    }, timeout);
...
async function callImageRecognition({
  ...
  return {
    response,
    // Shape/null/status/confidence validation is page-local; field meaning and
    // provenance wait for the final post-merge normalization gate.
  ...
  for (const auditRecord of audit.records) {
    ...
    for (const field of ["scene", "shot", "take", "takeStatus"]) {
      ...
      if (!primaryValue && auditValue) {
        ...
        if (!equivalentRecognitionField(field, primaryValue, auditValue)) {
          fields.push(field);
        }
      }
    }
...
// Page-local values remain raw until the final gate, so high-accuracy conflict
// checks compare canonical candidates without mutating either audit response.
function equivalentRecognitionField(field, left, right) {
  if (!["scene", "shot", "take"].includes(field)) return left === right;
  const leftCanonical = canonicalRecognitionValue(field, left);
  const rightCanonical = canonicalRecognitionValue(field, right);
...
function mergePageResults(pages, accuracyMode) {
  ...
  for (const page of pages) {
    ...
    for (const [index, record] of page.result.records.entries()) {
      // Assign identity only after page results have been combined. The global
      // index remains stable when recognition fields are corrected later.
      const recordIndex = records.length;
      records.push({
        ...record,
        // `id` remains the UI/editing identity; crop recheck must use the
        // stable final-sheet targetId instead of mutable card/video fields.
        id: `record-page-${page.pageNumber}-${index}`,
        sourcePage: page.pageNumber,
        targetId: recognitionTargetId(page.pageNumber, recordIndex),
      });
    }
    warnings.push(
...
function inheritSceneAndShot(records, warnings) {
  ...
  for (const { record, reel } of orderedRecords) {
    ...
    if (
      ...
      (!explicitScene || equivalentRecognitionField("scene", explicitScene, previous.scene))
    ) {
...
function repairSandwichedRows(group, reel, warnings) {
  for (let index = 1; index < group.length - 1; index += 1) {
    ...
    if (
      ...
      normalizedNumber(previous.record.shot) !== normalizedNumber(next.record.shot) ||
    ...
    if (
      normalizedNumber(current.record.shot) === normalizedNumber(expectedShot) &&
      normalizedNumber(current.record.take) === previousTake + 1
    ) {
      continue;
    }

    const oldValue = `${normalizedOrdinalLabel("shot", current.record.shot)}/${normalizedOrdinalLabel("take", current.record.take)}`;
    current.record.shot = expectedShot;
    current.record.take = expectedTake;
    // A deterministic sequence repair is useful for common dropped-digit
    // errors, but it is still an inference; keep the corrected cells visible
    // to the UI's manual-review affordance instead of presenting them as sure.
    lowerConfidence(current.record, ["shot", "take"]);
    warnings.push(
      `第 ${current.record.sourcePage || "?"} 页 ${reel} ${current.record.videoCode || "未知条号"} 位于连续条号与同镜次序之间，已将镜/次从 ${oldValue} 校正为 ${normalizedOrdinalLabel("shot", expectedShot)}/${normalizedOrdinalLabel("take", expectedTake)}，请人工复核。`,
    );
  }
}
...
function repairDroppedShotTens(group, reel, warnings) {
  for (let index = 1; index < group.length; index += 1) {
    ...
    const recognizedShot = normalizedOrdinalLabel("shot", current.record.shot);
    for (const item of run) {
      item.record.shot = correctedShot;
      lowerConfidence(item.record, ["shot"]);
    }
    warnings.push(
      `第 ${current.record.sourcePage || "?"} 页 ${reel} ${run[0].record.videoCode}–${run.at(-1).record.videoCode} 的镜号连续从 ${normalizedOrdinalLabel("shot", previous.record.shot)} 进入下一组，已将疑似漏写十位的 ${recognizedShot} 校正为 ${correctedShot}，请人工复核。`,
    );
    index += run.length - 1;
  }
}
...
function sameScene(...records) {
  ...
  const normalizedScenes = records.map((record) =>
    canonicalRecognitionValue("scene", record.scene, {
      fieldFormats: { scene: "XXX" },
    }),
  );
  return scenes.length === records.length &&
    normalizedScenes.every((scene) => scene != null && scene === normalizedScenes[0]);
}

function normalizedSceneLabel(value) {
  return canonicalRecognitionValue("scene", value, {
    fieldFormats: { scene: "XXX" },
  }) || String(value || "空");
}

function normalizedOrdinalLabel(field, value) {
  return canonicalRecognitionValue(field, value, {
    fieldFormats: { [field]: "XX" },
  }) || String(value || "空");
}

function normalizedNumber(value) {
  const normalized = canonicalRecognitionValue("shot", value, {
    fieldFormats: { shot: "XX" },
  });
...
function lowerConfidence(record, fields = []) {
  if (record.confidence === "high") record.confidence = "medium";
  if (fields.length) {
    record.reviewRequiredFields = orderReviewRequiredFields([
...
function requiresManualReview(record, field) {
  return reviewFieldsFromQuality(record).includes(field);
}

function videoCodeOrdinal(value) {
  const normalized = canonicalRecognitionValue("videoCode", value);
...
function normalizeReel(value) {
  return canonicalRecognitionValue("cardNumber", value) || "";
}
...
async function callOpenAI({
  ...
  const jsonMode = provider.chatJsonMode || provider.jsonMode || "json_schema";
  // JSON Object has no schema-bearing request field, so keep the same full
  // contract in the system message that prompt-only providers receive.
  const effectiveSystemPrompt = jsonMode === "prompt" || jsonMode === "json_object"
  ...
  if (jsonMode === "json_schema") {
    payload.text = { format: { type: "json_schema", name: "slate_sheet", description: "场记单结构化识别结果", strict: true, schema } };
...
async function callChatCompletions({
  ...
  const headers = optionalAuthHeaders(apiKey);
...
function optionalAuthHeaders(apiKey) {
  const headers = {};
  // Custom local endpoints can be intentionally unauthenticated. Omitting the
  // header entirely avoids gateways interpreting an empty Bearer as a key.
...
async function postChatCompletions(provider, payload, headers, fetchImpl, timeoutMs, signal) {
  ...
  if (choice?.error) {
    throw providerError(
      // Some gateways echo an Authorization value in a per-choice error;
      // derive the secret from the already-sanitized request headers rather
      // than relying on a key variable that is outside this helper's scope.
      redactSecret(choice.error.message || `${provider.label} 模型调用失败`, headerSecret(headers)),
      502,
    );
  }
  return data;
...
async function postJson(
  ...
  // Keep the same deadline active for headers and body bytes; Electron/undici
  // versions may surface either TimeoutError or AbortError.
  let raw;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
      ...
      raw = await readResponseTextWithDeadline(response, deadline.signal);
      const timedOut = deadline.didTimeout();
      deadline.cleanup();
      if (timedOut) throw new DOMException("模型请求超时", "TimeoutError");
      break;
    } catch (error) {
      const timedOut = deadline.didTimeout();
      deadline.cleanup();
      if (signal?.aborted) throw recognitionCanceledError();
      if (timedOut || isRequestTimeout(error)) {
        if (attempt < attempts) {
          console.warn(
      ...
      const detail = redactSecret(fetchErrorDetail(error), headerSecret(headers));
      console.error(
...
function normalizeBaseUrl(value, envName) {
  ...
  // URL normalisation can erase a trailing empty `?`/`#`; reject the raw
  // spelling too so dynamic providers receive the same strict validation as
  // the persisted configuration path.
