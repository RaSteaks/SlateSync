...
export async function recognizeSlate(
  ...
  // Electron project settings are the final output contract. Profile output
  // metadata remains available for legacy Web/MCP callers, but cannot silently
  // change Scene/Shot/Take formatting inside an isolated project.
  ...
  const pdfContext = withCustomPrompt(PDF_SYSTEM_PROMPT, customPrompt, slateCsvContext) + formatNote + scenarioNote;
  reportProgress({
    phase: "ocr",
    percent: 35,
