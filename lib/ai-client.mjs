  ...
  try {
    ocrResult = hasPdf
      ...
      : await ocrImpl(imageGroups, {
          env,
          // Local OCR implementations receive the same signal as model
          // requests so a stop action kills their worker before any model
          // request can be started with incomplete cancellation state.
          signal,
