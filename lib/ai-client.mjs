      ...
      try {
        ...
        if (capture) {
          capture.addPageResult(pageNumber, "audit", {
            request: { systemPrompt: auditContext, ocrEvidence: coreOcrEvidence, filename: `${pageFilename} · 核心字段查漏` },
        ...
        if (combined.conflicts.length || combined.auditOnly.length) {
          ...
          if (capture) {
            capture.addPageResult(pageNumber, "review", {
              request: { systemPrompt: reviewContext, userInstruction: `只复核以下 ${reviewTargets.length} 个素材键`, ocrEvidence: coreOcrEvidence, filename: `${pageFilename} · 冲突复核` },
