...
import { encryptionIdentifier, readProjectFile as readFile, writeProjectFile as writeFile } from "./local-encryption.mjs";
import {
  chmod,
  copyFile,
  lstat,
...
export async function exportProjectPackage(
  ...
  try {
    ...
    await backupSqliteTree(source, staging, source, { portable: true });
...
export async function exportProjectLibrary(sourcePath, targetPath) {
  ...
  try {
    await copyPortableTree(source, staging, { portable: true });
    await backupSqliteTree(source, staging, source, { portable: true });
...
async function copyPortableTree(
  source,
  target,
  { skipNames = new Set(), errorFactory = invalidLibrary, portable = false } = {},
) {
  ...
  // Portable exports omit machine-local key identifiers and decrypt JSON into
  // plaintext even when the destination inherits an ancestor marker. Imports
  // leave portable false so their destination library remains encrypted.
  if (basename(source) === ".slatesync-encryption" || isTransferTemporaryArtifact(basename(source))) return;
  if (stat.isDirectory()) {
    await mkdir(target, { recursive: true, mode: stat.mode });
    for (const entry of await readdir(source)) {
      await copyPortableTree(join(source, entry), join(target, entry), { skipNames, errorFactory, portable });
    }
    return;
  }
  ...
  if (source.endsWith(".json")) {
    ...
    await (portable ? writeRawFile : writeFile)(target, contents, { mode: 0o600 });
  }
  else await copyFile(source, target);
}

async function backupSqliteTree(sourceRoot, targetRoot, directory = sourceRoot, { portable = false } = {}) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      await backupSqliteTree(sourceRoot, targetRoot, source, { portable });
      continue;
    }
    if (!entry.isFile() || !entry.name.endsWith(".sqlite")) continue;
    const target = join(targetRoot, relative(sourceRoot, source));
    await mkdir(dirname(target), { recursive: true });
    const db = Database(source, { readonly: true, fileMustExist: true });
    try {
      // Imports staged beneath an encrypted library inherit its destination
      // key; explicit portable exports override any ancestor marker.
      if (portable) await db.backup(target, { portable: true });
      else if (encryptionIdentifier(target)) await writeFile(target, db.serialize(), { mode: 0o600 });
      else await db.backup(target);
      // 备份可能继承源库的 WAL 模式；项目包必须是单文件数据库，先切回
      // DELETE journal 再关闭副本，避免校验时重新生成 -wal/-shm 文件。
      const portableDb = Database(target);
      try {
...
async function readProjectStorage(rootPath, suppliedProject = null) {
  ...
  let db;
  try {
    db = Database(join(rootPath, SQLITE_FILENAMES.project), {
      readonly: true,
      fileMustExist: true,
    });
...
async function writeJsonAtomic(path, value, { portable = false } = {}) {
  // 包内 JSON 先写临时文件再替换；portable 导出使用原始写入，避免
  // 暂存目录继承加密祖先后把最终可移植清单重新加密。
  const temporaryPath = `${path}.${randomUUID()}.tmp`;
  await (portable ? writeRawFile : writeFile)(temporaryPath, JSON.stringify(value, null, 2), {
    encoding: "utf8",
    mode: 0o600,
  });
