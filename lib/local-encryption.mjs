...
let bridge;
let resourcesRoot;
// Electron exposes process.resourcesPath as read-only in Workers. Pass its
// trusted application resource location explicitly, without mutating process.
export function configureLocalEncryptionResources(path) {
  if (bridge) throw new Error("加密资源路径必须在打开数据库之前配置");
  resourcesRoot = path;
}
function nativeBridge() {
  ...
  try {
    ...
    const root = existsSync(join(localRoot, "bin/local-encryption.dylib"))
      ? localRoot : join(resourcesRoot || process.resourcesPath || localRoot, "app");
