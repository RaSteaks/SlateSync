...
export async function runVisionOcrForPages(imageDataGroups, options = {}) {
  ...
  const outputSettings = {
    ...
    alternativesCount: status.alternativesCount,
    outputSchemaVersion: status.outputSchemaVersion,
    includeCrops: Boolean(options.includeCrops),
  };
...
export function visionOcrPublicConfig(env = process.env, options = {}) {
  ...
  const alternativesCount = Math.trunc(numberSetting(env.VISIONOCR_ALTERNATIVES, 0, 0, 3));
  const outputSchemaVersion = alternativesCount > 0 ? "vision-ocr-v2-alternatives" : "vision-ocr-v1";
...
function normalizeOcrResult(response, status, options = {}) {
      ...
      ? page.views.map((view, viewIndex) => ({
          ...
          truncated: Boolean(view.truncated),
            blocks: Array.isArray(view.blocks)
              ? view.blocks.map((block, blockIndex) => normalizeBlock(block, blockIndex, status.alternativesCount, Boolean(options.includeCrops))).filter(Boolean)
            : [],
        }))
      : [],
  }));
  return {
    ...
    alternativesEnabled: status.alternativesCount > 0,
    alternativeCount: status.alternativesCount,
    outputSchemaVersion: response.outputSchemaVersion || status.outputSchemaVersion,
...
function normalizeBlock(block, index, alternativesCount = 0, includeCrops = false) {
  ...
  const alternatives = alternativesCount > 0 && Array.isArray(block.alternatives)
    ? block.alternatives
      .map((candidate) => ({
        text: clean(candidate?.text).slice(0, 160),
        confidence: Math.max(0, Math.min(1, Number(candidate?.confidence) || 0)),
      }))
      .filter((candidate) => candidate.text && candidate.text !== text)
  ...
  return {
    ...
    ...(includeCrops && clean(block.cropImage || block.cropDataUrl) ? { cropImage: clean(block.cropImage || block.cropDataUrl) } : {}),
  };
}
...
function documentCacheKey(imageDataGroups, status, outputSettings) {
  ...
  hashSizedValue(hash, String(outputSettings.alternativesCount));
  hashSizedValue(hash, String(outputSettings.outputSchemaVersion));
  hashSizedValue(hash, String(Boolean(outputSettings.includeCrops)));
