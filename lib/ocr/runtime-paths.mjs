...
// WP-2 keeps one tracked PaddleOCR subtree. The temporary Electron oracle uses
// that source in development and mirrors the same subtree into its packaged
// resource root until WP-6 removes the oracle.
export function paddleResourceDir(env = process.env) {
  const projectDir = runtimeProjectDir(env);
  return isPackagedRuntime(env)
    ? join(projectDir, "PaddleOCR")
    : join(projectDir, "SlateSyncApp", "Resources", "PaddleOCR");
}
