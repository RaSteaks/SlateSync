  ...
  for (const view of page.views) {
    ...
    for (const block of selected) {
      const box = (block.bboxNormalized || [])
        ...
        .join(",");
      const alternatives = Array.isArray(block.alternatives) && block.alternatives.length
        ? ` alternatives=${JSON.stringify(block.alternatives.slice(0, 3))}`
        : "";
      const line = `#${block.order} q=${Number(block.confidence).toFixed(3)} box=[${box}] text=${JSON.stringify(block.text)}${alternatives}`;
...
export function summarizeOcrResult(result = {}) {
  ...
  return {
    ...
    alternativesEnabled: Boolean(source.alternativesEnabled || Number(source.alternativesCount) > 0),
    alternativeCount: Number(source.alternativesCount || source.alternativeCount) || 0,
    outputSchemaVersion: source.outputSchemaVersion || null,
    ...
    views: Array.isArray(page.views)
      ? page.views.map((view, viewIndex) => ({
          ...
          blocks: Array.isArray(view.blocks)
            ? view.blocks.map((block, blockIndex) => normalizeBlock(block, blockIndex, Boolean(options.includeCrops))).filter(Boolean)
            : [],
        }))
      : [],
  }));
...
function normalizeBlock(block, index, includeCrops = false) {
