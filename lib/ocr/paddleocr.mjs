...
export async function runPaddleOcrForPages(imageDataGroups, options = {}) {
  ...
  try {
    ...
    const response = await execute(payload, {
      ...
      runnerPath: options.runnerPath || RUNNER_PATH,
...
export function preloadPaddleOcr(env = process.env, options = {}) {
  ...
  return scheduleWorkerOperation(async () => {
    throwIfRecognitionCanceled(options.signal);
...
// The app exits without awaiting Electron's event loop. Force mode therefore
// terminates the native process immediately; normal mode drains through the
// same operation queue so a settings change never frees a live model mid-task.
export async function closePaddleOcrWorker({ force = false, deadlineAt = null } = {}) {
  if (force) {
    ...
    const child = spawn(python, [RUNNER_PATH, "--check"], {
      ...
      env: {
        ...
        PADDLE_PDX_CACHE_HOME:
          env.PADDLE_PDX_CACHE_HOME || DEFAULT_MODEL_CACHE_DIR,
      },
    });
...
function workerConfigKey(payload, options) {
  ...
  return JSON.stringify({
    ...
    runnerPath: options.runnerPath || RUNNER_PATH,
    cacheHome: runnerEnv.PADDLE_PDX_CACHE_HOME || DEFAULT_MODEL_CACHE_DIR,
...
async function executeRunner(payload, options) {
  ...
  try {
    return await scheduleWorkerOperation(async () => {
      throwIfRecognitionCanceled(options.signal);
    ...
    if (paddleWorkerStopRequested) throw error;
...
function createPaddleWorker(key, options) {
  const runnerEnv = options.env || process.env;
  const child = spawn(options.pythonPath || "python3", [options.runnerPath || RUNNER_PATH, "--server"], {
    cwd: PROJECT_DIR,
    ...
    env: {
      ...
      PADDLE_PDX_DISABLE_MODEL_SOURCE_CHECK: "True",
      PADDLE_PDX_CACHE_HOME:
        runnerEnv.PADDLE_PDX_CACHE_HOME || DEFAULT_MODEL_CACHE_DIR,
    },
  });
  const worker = {
    key,
...
async function executeOneShotRunner(payload, options) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(runnerOptions.pythonPath, [runnerOptions.runnerPath], {
      cwd: PROJECT_DIR,
      stdio: ["pipe", "pipe", "pipe"],
      env: {
        ...runnerEnv,
