...
export function openSlateDatabase(baseDir, options = {}) {
  ...
  try {
    ...
    // A no-op DDL transaction still rewrites an authenticated database in full.
    // Inspect first; only a missing schema object/column enters a write transaction.
    const schema = kind === "library" ? LIBRARY_SCHEMA : PROJECT_SCHEMA;
    const expected = [...schema.matchAll(/CREATE (?:TABLE|INDEX) IF NOT EXISTS (\w+)/g)].map((match) => match[1]);
    const existing = new Set(db.prepare("SELECT name FROM sqlite_master WHERE type IN ('table', 'index')").all().map((row) => row.name));
    const missingObjects = expected.some((name) => !existing.has(name));
    const missingSummary = kind === "project" && existing.has("tasks") &&
      !db.prepare("PRAGMA table_info(tasks)").all().some((column) => column.name === "summary_json");
    if (missingObjects || missingSummary) {
      db.transaction(() => {
        db.exec(schema);
...
export async function migrateJsonDirectory({
  ...
  const transaction = db.transaction((records) => {
    ...
    // Derived metadata must commit (or roll back) with the newly imported rows.
    // Empty/duplicate-only migrations do not invalidate a still-correct cache.
