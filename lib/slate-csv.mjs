// Parse slate CSV exported from the on-set slate management system.
// The CSV contains high-confidence Scene/Shot/Take/Comments columns.
// File Name is used only to extract reel+clip for matching with Resolve CSV,
// NOT as a source of slate metadata.
...
export function parseSlateCsv(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Skip rows with no scene/shot/take at all
...
export function formatSlateCsvForPrompt(records) {
