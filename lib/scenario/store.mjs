...
//
// Profiles are immutable structure snapshots. Matching updates usage metadata
// and records an observation, while a changed layout creates a new profile.
...
export function createScenarioStore(baseDir, options = {}) {
  ...
  const store = {
    ...
    async matchAndSave(observation, options = {}) {
      ...
      // config applies consistently to Web, Electron, and MCP recognition.
      const matching = {
        ...DEFAULT_SCENARIO_MATCHING,
