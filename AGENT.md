...
2026-09-16：优化多任务项目打开、SQLite 统计与任务轨道 GUI。缓存无成功识别
记录的查询结果；拆分 COUNT/MAX；迁移事务内失效缓存并在迁移完成后生成项目
摘要；任务搜索延迟计算并复用索引、日期格式化器，虚拟行按任务 ID 计量。
仅使用临时合成库与无头浏览器验证，详见 `docs/project-open-performance-2026-09-16.md`。

2026-09-16：实施运行逻辑与存储性能 Review 修复：自动保存失败保留最新快照；
项目共享初始化 Promise、操作租约和有界闲置回收；数据库初始化避免无效 DDL；
SQLite/加密/项目库传输由单一后台存储 Worker 持有；窗口同意退出后排空写入。
保留 Swift 加密和 Renderer IPC 契约，全程仅使用临时合成库、无前台窗口验证。
详细结果见 `docs/runtime-storage-review-2026-09-16.md`。
...
## 2026-09-16 运行逻辑与存储 Worker 修复

- 自动保存失败不再用旧快照覆盖保存期间产生的新版本；重试保持最新内容，
  只有当前版本落盘后才清除 dirty 状态，保留任务切换和迟到结果隔离。
- 项目 runtime 在异步初始化前登记共享 Promise；失败关闭唯一句柄并允许重试。
  每项读写和完整识别流程持有租约，取消确认等待 Worker 租约释放。最多缓存
  1 个闲置项目，60 秒后回收，活动项目豁免；初始化、关闭和回收互斥。
- 新增内部白名单 RPC 与异步 store 代理，生产 SQLite、加密及导入导出由唯一
  Node Worker 执行。Worker 串行处理包括 JSON 快照在内的完整命令，出错保留
  错误 code，崩溃拒绝待处理请求且不重放写入；密钥不跨 RPC、不进入 Renderer。
- 打开数据库先检查表/索引/列，仅缺失时执行一个 additive 迁移事务；已有 schema
  不再触发整库加密重写。项目列表顺序读取并关闭临时库，限制解密连接峰值。
- 退出请求先取消识别；Renderer 可在 beforeunload 阻止退出并继续保存。所有窗口
  同意关闭后，由 will-quit.preventDefault 暂缓进程退出，停止 IPC 接收、排空存储
  与日志后重新 quit。库切换也走该路径，不再用固定延时 app.exit 打断写入。
- Electron Worker 的 resourcesPath 是只读属性，采用加密模块显式资源路径配置。
  临时 ASAR 已验证普通/合成加密库、Worker ESM 路径、SQLite 原生模块及 dylib 加载。
- 验证：Node 519/519；Modern 34 文件 215/215；Electron Node-only ABI 专项
  74/74（与 Node 用例有重叠）；TypeScript、check、production build、diff 检查通过。
  恢复了任务开始时的 Node SQLite 原生二进制；没有运行 GUI/E2E 或访问真实项目库。
- 32/128 MiB 合成加密库各进行 3 次小保存，对比同一存储实现直接调用与 Worker：
  调用线程最大 5 ms 定时器间隔分别由 36.47/114.43 ms 降至 5.70/5.96 ms。
  真实写入仍需整库序列化、加密和 fsync；测试报告明确 CPU/RSS 是全进程值，
  不等同于真实应用峰值或端到端 GUI 性能，不声称消除了全库写入/内存成本。

### 2026-09-16 Review 后续修复

- 项目库操作先检查项目/全局设置的 dirty 与 saving 状态。目标路径落盘和
  `app.relaunch()` 延后到窗口接受退出、IPC 排空后；`will-prevent-unload` 尊重
  草稿保护，取消待激活目标并释放传输锁。改名取消退出后保存已改名的当前路径，
  导入/更换位置取消退出则保留源库路径，避免后续保存被 LIBRARY_BUSY 永久阻断。
- shutdown 同时取消所有模型探针；同一 tick 已受理、尚未进入的探针也立即观察
  aborted signal，并等待已完成探测结果的缓存持久化结束。
- Main 读取、写入、识别复用外层租约上下文；Worker 内层 store 租约仅检查访问并
  固定连接，不重复生成摘要。临时草稿项目快照测试验证冷/热请求摘要均为 1 次。
- 本轮验证：完整 Node 525/525；Modern 34 文件 216/216；TypeScript、静态检查、
  生产构建、git diff --check 通过。新增退出否决/恢复、探针退出竞争、摘要计数和
  设置草稿门禁回归测试。全部后台执行，未打开 GUI，未访问真实库，未提交/推送。
- 本轮未重复大库性能基准与 Electron ABI/ASAR 专项；前述数字属于前一轮实测。
  全库加密写入成本仍然存在；现有构建大 chunk 与 jsdom 提示仍在，测试无失败。
