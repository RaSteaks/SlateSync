...
## 2026-09-25 Provider 审查修复（第二批）

- 凭据文件操作改由专用串行 DispatchQueue 执行，保留跨进程排他锁；Mutex 协调取消与 continuation 所有权。排队和等锁可取消，提交开始后返回真实结果；不缓存明文，不修改现有 AES-GCM 文件格式。
- 增加 statuses(for:) 批量状态接口，一次锁和解密生成全部 Provider 状态；取消传播，锁超时、访问失败及无法解密分别投影。瞬态和权限错误不建议重置，内建/自定义界面复用恢复提示。
- 删除旧 Provider 迁移入口、快照字段、无效初始化参数和旧存储 actor/解析器；KeychainBackend 独立保留给项目加密。保留共享后端与项目授权回归，只移除旧 Provider 专属测试；不删除用户旧文件或钥匙串条目。
- 验证：定向 42 项通过；全套 SwiftPM 415 项、2 项跳过、零失败。随后新增真实跨进程持锁回归，最终定向 23 项通过；原生 App Debug 无签名构建通过，本地化审计与 git diff --check 通过。网页 strict 审计仍为历史原型/文档中的 69 项，本轮未变更；未进行原生 UI 手动验收。

## 2026-09-25 Provider 审查修复（第一批）

- 搜索无结果仅在存在有效关键词时显示，避免空目录同时出现两个矛盾空态；删除未引用的备用组合 Section，修正帮助文案空格。
- 自定义 Provider 保存保留经过 ProductPrivacy 清理的错误码和原因；密钥写入与状态刷新分别处理，刷新失败保留 credentialSaved=true，编辑器清空已成功保存的密钥。取消单独显示取消状态。
- 删除仅由测试调用的 addCustomProvider/saveCustomProvider 两条旧入口；验证与规范化辅助函数继续供唯一的 saveCustomProviderConfiguration 生产路径使用，编辑身份和修订回归转到该路径。
- 本批曾将凭据 I/O 队列、错误分类、批量状态以及旧迁移/Keychain 拆分方向提请确认；用户确认后的实施见上方第二批记录。
- 验证：ProviderManagementTests 与 SM08OwnershipTests 共 66 项通过；本地化审计与 git diff --check 通过。通用网页 strict 审计在历史 HTML 原型及文档中报告 69 项，本轮未修改这些网页，不将其记为通过。未做原生界面手动验收。

## 2026-09-25 API Key 内存驻留收紧

- Provider API Key 继续保存在 macOS 钥匙串；凭据服务仅合并在途读取，每次请求重新读取，不再跨请求缓存明文。状态查询只读属性；并发替换、删除及授权拒绝继续按既有修订与等待顺序处理。
- 设置中的删除确认明确仅移除本机副本，服务商平台的密钥注销仍由用户完成。API Key 不回显，也不进入项目包、配置或日志；项目库 AES-GCM 方案保持现状。
- 验证：凭据定向 5 项、离线网络定向 12 项、本地化定向 8 项、严格 SwiftPM 构建、本地化审计、SM-09 资源契约和一次性钥匙串签名探针通过。完整 410 项套件在受限环境中有 2 项无关系统测试失败（Vision OCR 服务和安装器子进程）；两项在允许对应系统能力的环境中分别复测通过，不将受限环境的完整运行记为 PASS。

## 2026-09-24 Provider 审查修复

- 内建 Provider 的探针结果以兼容旧 v2 文件的可选缓存字段持久化，独立于模型发现列表；重启恢复时校验实际地址、协议、图像设置、请求头及兼容模型配置。
- 单模型探针只更新对应物理模型的成功或失败结果，保留其他模型的验证；目录公开别名和 API ID 解析到同一结果，旧任务选择继续有效。
- 请求配置变更移除持久化验证；凭据写入前先撤销验证，即使 Keychain 拒绝也不能保留旧证明。默认、备用及其他无关设置保存保留验证。
- 新增离线集成回归，覆盖逐个验证、发现刷新、主备组合重启、旧别名、失败重验、配置和凭据失效、旧 v2 解码及迟到结果拒绝。
- 探针运行时关闭等待结果持久化结束，避免凭据更新与旧证明写入竞态。验证：全套 `swift test` 408 项、2 项专用环境跳过、零失败；收尾修改后针对性复测 46 项全部通过。

## 2026-09-24 Better Provider 实施记录

- 按 `.claude/provider/provider-plan.md` 增加 11 个仅用于预填的 Provider 预设；自定义配置的备注与预设来源为可选兼容字段，凭据仍由 Keychain 管理。
- 全局默认和有序备用按完整 Provider/模型组合保存。配置阶段探针记录成功 JSON 档位；影响请求的配置与凭据变更使旧验证失效，名称与备注编辑保留验证。
- 识别输出只做文本修复、宽松解码和降级报告；原始响应不改写。逐页故障转移只派发已验证备用，保留成功页用于同任务重试，高精度失败页整组重发；服务级故障隔离当前队列批次。
- 设置使用统一 Provider 列表、能力状态徽章、预设选择器、默认及备用组合编辑；工作台可设置已验证组合为全局默认。界面沿用 `DESIGN.md` 的冷灰与钨丝琥珀语义色。

## 2026-09-24 Provider 方案方向修订（尚未实施）

- 详细方案见 `.claude/provider/provider-plan.md`：任务只选择主 Provider/模型，全局设置维护已验证的备用组合。
- 超时预算耗尽后排队任务转备用，保留成功页，失败页完整重发；迟到的旧尝试不得覆盖结果。主备全失败提示用户，不展示或永久记录逐页 Provider 来源。
- 验证在配置阶段完成，本轮不增加 Anthropic 原生协议。本次仅修订文档，未实施功能。
...
## 2026-09-25 Provider 管理与本地加密凭据

- Provider 存储切换为 `EncryptedFileCredentialStore`：Application Support/SlateSync/Credentials
  下的 `provider-keys.enc` 与独立 `master.key`，CryptoKit AES-256-GCM、随机 256 位主密钥、
  每次 seal 新 nonce、认证版本头、0700/0600 权限。文件锁内读取/修改/原子替换，
  拒绝链接与非普通文件；损坏或丢失主密钥时不覆盖数据。
- Runtime 与工作流统一使用文件存储；保留旧接口兼容但不执行旧凭据迁移，不读取或
  删除旧钥匙串与旧 provider-keys.json。用户重新填写 API Key，环境变量配置维持原语义。
  项目库加密仍使用钥匙串，不能承诺项目库操作完全无授权提示。
- 独立本地主密钥允许无密码读取，同时拿到密文和主密钥即可解密，不声称具有钥匙串
  应用隔离。两文件不进入项目导出、日志或 UI projection。重置为明确确认的恢复动作。
- Provider 页新增搜索、已添加/折叠内建分组、卡片与行内状态；添加流程统一预设与配置，
  配置页包含隐藏的 API Key 输入与高级参数。自定义保存返回分阶段结果和稳定身份，
  密钥失败可重试而不重复创建，其他设置草稿保留。默认/备用/删除仍需页面保存。
- 更新注释、双语帮助、本地化与存储文档；测试采用临时根和模拟密钥。
- 验证：完整 SwiftPM 418 项、2 项既有跳过、0 失败；最终 UI/存储/工作流定向回归另行运行。
  Xcode Debug 构建与隔离启动成功。实机隔离根 `/tmp/slatesync-provider-ui-check` 中完成
  预设新增、模拟 API Key 保存、重启读取、搜索/清除、卡片内错误和 700×540 尺寸检查，
  未出现 Provider 钥匙串授权。隔离环境禁止外部服务，未声称实际付费模型连接验证通过。
  本地化审计无缺失；通用 premium 静态审计的 69 项告警来自现有 HTML 文档/原型，
  未因本次原生界面任务扩大修改其交互。
- 补充回归发现并修复重置后内存模型证明未失效的问题：除磁盘能力缓存外同步失效共享
  ProviderRegistry，并撤销 UI 在途请求身份。最终 26 项定向回归全部通过；英文浅色与
  中文深色实机布局通过，用户备注作为用户内容保留原文。
