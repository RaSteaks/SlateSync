...
| Xcode Debug build | PASS | 产品提交 `cf43968`；`/private/tmp/slatesync-sm08-cf43968-debug.log` 与 `/private/tmp/slatesync-sm08-cf43968-xcode-debug`，`** BUILD SUCCEEDED **` |
| Xcode static analysis | PASS | 产品提交 `cf43968`；`/private/tmp/slatesync-sm08-cf43968-analyze.log` 与 `/private/tmp/slatesync-sm08-cf43968-xcode-analyze`，`** ANALYZE SUCCEEDED **`，无 SlateSync 源码诊断 |
| Xcode Release build | PASS | 产品提交 `cf43968`；`/private/tmp/slatesync-sm08-cf43968-release.log` 与 `/private/tmp/slatesync-sm08-cf43968-xcode-release`，`** BUILD SUCCEEDED **` |
| Release Archive | PASS_LOCAL | 产品提交 `cf43968`；`/private/tmp/slatesync-sm08-cf43968-archive.log` 与 `/private/tmp/slatesync-sm08-cf43968-signed.xcarchive`，arm64/x86_64 universal、ad hoc、hardened runtime，`codesign --verify --deep --strict` 通过 |
...
同时影响 WindowGroup 与 Settings。最终实现继续替换该组，但用 SwiftUI
`dismissWindow()` 显式恢复当前窗口 ⌘W，并只保留一个聚焦 ⌘S；工具栏 Save 不再
重复注册快捷键。该根因有直接历史证据且可后台编译，但修复后的真实 ⌘W/⌘S
交互仍需在允许前台的专用 UI 环境中复验，暂不改记为 PASS。
...
产品提交 `cf4396856e55f485ddcf64439284e75de3be3d35` 的 Archive 为 universal
