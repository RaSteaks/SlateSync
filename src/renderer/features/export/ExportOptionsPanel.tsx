...
import { CUSTOM_EXPORT_OPTIONS, mergeExportOptions, normalizeExportOptions } from "../../../../public/export-options.js";
  ...
  return <div className={styles.exportOptionsPanel} data-export-options-source={sourceLabel || undefined}>
    ...
    <div className={styles.exportColumnList} aria-label="导出列顺序">
      ...
      {normalized.columns.map((column, index) => <div className={styles.exportColumnRow} key={column.key}>
        ...
        <Input aria-label={`${labelFor(column.key)} 列标题`} value={column.header} readOnly={builtin || imported} onChange={(event) => patch({ columns: normalized.columns.map((item) => item.key === column.key ? { ...item, header: event.target.value } : item) })} disabled={disabled} />
        <Button type="button" variant="ghost" size="sm" aria-label={`上移${labelFor(column.key)}`} onClick={() => moveColumn(index, -1)} disabled={disabled || index === 0} startIcon={<ArrowUp size={14} />} />
        <Button type="button" variant="ghost" size="sm" aria-label={`下移${labelFor(column.key)}`} onClick={() => moveColumn(index, 1)} disabled={disabled || index === normalized.columns.length - 1} startIcon={<ArrowDown size={14} />} />
      </div>)}
    </div>
  </div>;
}
