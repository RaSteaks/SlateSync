...
export function ProjectLibraryPage({ onOpenProject }: { onOpenProject: (id: string, route?: "workspace" | "project-settings") => void }) {
  ...
  return <div className={styles.page}>
    ...
    <div className={`${styles.grid} ${styles.gridTwo}`} style={{ marginBottom: 20 }}>
      <Surface tone="accent" compact className={styles.metric}><span className={styles.kicker}>当前项目</span><strong className={styles.metricValue}>{active.length}</strong><span className={styles.metricNote}>可继续工作</span></Surface>
    ...
    <Surface className={styles.panel}>
      <div className={styles.sectionHeader}><div><p className={styles.kicker}>项目</p><h2 className={styles.sectionTitle}>当前项目</h2></div><Stack direction="row" gap={2} align="center"><Button variant="ghost" size="sm" startIcon={<Import size={15} />} onClick={() => void runLibraryAction("import")} loading={actionBusy === "import"}>导入</Button><Button variant="ghost" size="sm" startIcon={<PackageOpen size={15} />} onClick={() => void runLibraryAction("export")} loading={actionBusy === "export"}>导出</Button><Button variant="ghost" size="sm" startIcon={<MapPin size={15} />} onClick={() => void runLibraryAction("change")} loading={actionBusy === "change"}>更换位置</Button></Stack></div>
...
function ProjectCard({ project, current, busy, onOpen, onSettings, onArchive }: { project: import("../../../shared/contracts/index.js").ProjectSummary; current: boolean; busy: boolean; onOpen: () => void; onSettings: () => void; onArchive: () => void }) {
  ...
  return <Surface compact className={styles.projectCard} data-current={current || undefined} data-archived={archived || undefined}>
    <div className={styles.projectTop}><button type="button" onClick={onOpen} style={{ display: "contents" }} aria-label={`打开项目 ${project.name}`}><span className={styles.projectMark}><FolderKanban size={19} /></span><div style={{ flex: 1, minWidth: 0 }}><h3 className={styles.projectName}>{project.name}</h3>{project.description && <p className={styles.projectDescription}>{project.description}</p>}</div></button></div>
